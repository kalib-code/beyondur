...
export const Spaces: NextPage<Props> = props => {
  ...
  return (
    <div className="bg-dotted-spacing-4  bg-dotted-gray-300">
      ...
      <NavBar />
      <div className="container mx-auto my-20 grid place-items-center  ">
        <div>
          <h1 className="text-6xl font-bold text-accent">{space?.title}</h1>
        </div>
        <div>
          <h1 className="font-base text-2xl text-accent">{space?.message}</h1>
        </div>
      ...
      <div className="container mx-auto my-20 ">
        <div className="columns-1xs h-full columns-4 gap-3 ">
          {testimonialsData.map((testimonial, index) => {
            return (
              <>
                {testimonial.isVideo ? (
                  <LoveCardsVideo key={index} {...testimonial} />
                ) : (
